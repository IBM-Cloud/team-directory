...
if (!process.env.REDIS_URL || !process.env.REDIS_CERT) {
  console.error('Redis variables missing. Please create Redis environment variables before running. See README for details.')
  process.exit(1)
}
...
const cert = Buffer.from(process.env.REDIS_CERT, 'base64').toString('utf-8')
...
const populateDb = async function () {
  ...
  try {
    ...
    console.log("Failed to create database and indexes:", error)
    return
  }
}
